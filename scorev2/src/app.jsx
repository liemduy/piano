...
function App() {
            ...
            }, [buildSheetTarget, openRangeMenu]);
            ...
            const selectAbcRange = useCallback((startChar, endChar) => {
                ...
                // Focus + select
                try { ta.focus({ preventScroll: true }); } catch (e0) { try { ta.focus(); } catch (e1) {} }
                try { ta.setSelectionRange(s, e); } catch (e2) {}
            ...
            const buildSheetTarget = useCallback((abcElem, analysis) => {
                ...
                return buildTimelineTarget(s, e);
            }, [buildTimelineTarget]);
            ...
            const sheetClickListener = useCallback((abcElem, tuneNumber, classes, analysis, drag, mouseEvent) => {
                try {
                    ...
                    const target = buildSheetTarget(abcElem, analysis);
                    if (!target) return;

                    selectAbcRange(target.startChar, target.endChar);
                } catch (e) {}
            }, [buildSheetTarget, selectAbcRange]);
            ...
            const renderSheetMusic = useCallback((text) => {
                ...
                try {
                    ...
                    if (visualObjRef.current && typeof visualObjRef.current.setUpAudio === 'function') {
                        try { sequenceRef.current = visualObjRef.current.setUpAudio(); } catch (e) {}
                    }
                    if (visualObjRef.current && typeof visualObjRef.current.setTiming === 'function') {
                        try { noteTimingsRef.current = visualObjRef.current.setTiming(); } catch (e) {}
                    }
            ...
            const syncFullLoopState = (sc) => {
                if (!sc || typeof sc.toggleLoop !== 'function') return;
                if (!!sc.isLooping !== !!loopEnabledRef.current) {
                    try { sc.toggleLoop(); } catch (e) {}
                }
            };
            ...
            const handleRangePlaybackEnded = (shouldLoop) => {
                const state = rangePlaybackRef.current;
                stopRangeTimer();

                if (shouldLoop && rangeSelectionRef.current && rangeSelectionRef.current.type === 'active') {
                    clearPlaybackHighlights();
                    state.pausedMs = 0;
                    state.currentMs = 0;
                    state.nextEventIndex = 0;
                    ...
                    try {
                        rangeSynthRef.current && rangeSynthRef.current.stop();
                        rangeSynthRef.current && rangeSynthRef.current.start();
                        state.ignoreEnded = false;
                        startRangeTimer(0);
                        playbackModeRef.current = 'range';
                        setIsPlaying(true);
                        return;
                    } catch (e) {
                ...
                state.nextEventIndex = 0;
                state.ignoreEnded = true;
                try { rangeSynthRef.current && rangeSynthRef.current.stop(); } catch (e) {}
                playbackModeRef.current = null;
                setIsPlaying(false);
            };

            const prepareRangePlayback = async () => {
                ...
                await synth.init({
                    ...
                    onEnded: () => {
                        if (rangePlaybackRef.current.ignoreEnded) return;
                        handleRangePlaybackEnded(loopEnabledRef.current);
                    },
                });
                await synth.prime();
            ...
            const startRangePlayback = async () => {
                ...
                const payload = await prepareRangePlayback();
                if (!payload || !rangeSynthRef.current) return;

                const state = rangePlaybackRef.current;
                const resumeMs = state.pausedMs > 0 ? state.pausedMs : 0;
                ...
                try {
                    ...
                    console.warn('Range audio problem:', err);
                }
            };

            const startFullPlayback = () => {
                if (!window.ABCJS) return;
                const sc = ensureSynthController();
                if (!sc) return;
                ...
                sc.setTune(visualObjRef.current, true, audioParams).then(() => {
            ...
            const togglePlay = () => {
                ...
                if (isPlaying) {
                    if (playbackModeRef.current === 'range') pauseRangePlayback();
                    else {
                        const sc = synthControlRef.current;
                        if (sc) {
                            try { sc.pause(); } catch (e) {}
            ...
            useEffect(() => {
                if (playbackModeRef.current === 'full') {
                    syncFullLoopState(synthControlRef.current);
                }
            }, [loopEnabled]);
