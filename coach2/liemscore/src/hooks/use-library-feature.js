    ...
    function useLibraryFeature(options) {
        ...
        useEffect(() => {
            ...
            const { data: authListener } = supabaseClient.auth.onAuthStateChange((event, session) => {
                ...
                const shouldRefreshLibrary = event === 'INITIAL_SESSION' || event === 'SIGNED_IN' || event === 'USER_UPDATED' || event === 'PASSWORD_RECOVERY';
                window.setTimeout(() => {
                    if (!user?.id) {
                        setLibraryItems([]);
                        clearLinkedSong();
                    ...
                    if (!shouldRefreshLibrary) return;

                    fetchLibrary(user.id).catch((error) => {
                        console.error('Deferred auth library refresh failed', error);
                    });
                }, 0);
            });
