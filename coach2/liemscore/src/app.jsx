...
function App() {
            ...
            const [playbackStyle, setPlaybackStyle] = useState('default');
            ...
            const [headerScaledHeight, setHeaderScaledHeight] = useState(56);
            ...
            useEffect(() => {
                ...
                const measure = headerMeasureRef.current;
                if (!shell || !measure || typeof ResizeObserver === 'undefined') return undefined;

                let rafId = 0;
                const updateHeaderLayout = () => {
                    rafId = 0;
                    const shellWidth = Math.max(0, shell.clientWidth - 4);
                    const contentWidth = Math.max(measure.scrollWidth, measure.offsetWidth, 1);
                    const baseHeight = Math.max(measure.scrollHeight, measure.offsetHeight, 56);
                    const nextMode = shellWidth < 980 ? 'micro' : shellWidth < 1180 ? 'tiny' : shellWidth < 1420 ? 'compact' : 'full';
                    let nextScale = 1;
                    if (contentWidth > shellWidth) nextScale = Math.max(0.52, Math.min(1, shellWidth / contentWidth));
                    const nextScaledHeight = Math.max(38, Math.ceil(baseHeight * nextScale));
                    setHeaderMode(prev => (prev === nextMode ? prev : nextMode));
                    setHeaderScale(prev => (Math.abs(prev - nextScale) < 0.01 ? prev : nextScale));
                    setHeaderScaledHeight(prev => (Math.abs(prev - nextScaledHeight) < 1 ? prev : nextScaledHeight));
                };

                const schedule = () => {
                    if (rafId) cancelAnimationFrame(rafId);
                    rafId = requestAnimationFrame(updateHeaderLayout);
                };
            ...
            const applyTextChange = (newText, newPos, { skipHistory = false, focus = true } = {}) => {
                ...
                const prevScrollTop = ta ? ta.scrollTop : 0;
                const prevScrollLeft = ta ? ta.scrollLeft : 0;
                ...
                setTimeout(() => {
                    ...
                    if (focus) {
                        ...
                        try { node.setSelectionRange(newPos, newPos); } catch (e) {}
                    }
                    try { node.scrollTop = prevScrollTop; } catch (e) {}
                    try { node.scrollLeft = prevScrollLeft; } catch (e) {}
                }, 0);
            };
            ...
            useEffect(() => {
                ...
                const resolveSheetClick = (e) => {
                    if (typeof e.button === 'number' && e.button !== 0) return false;
                    const visual = visualObjRef.current;
                    if (!visual || typeof visual.findSelectableElement !== 'function') return false;
            ...
            const scrollAbcSelectionIntoView = useCallback((node, start, end) => {
                if (!node) return;
                try {
                    ...
                    const props = [
                        ...
                        'lineHeight', 'textTransform', 'textIndent', 'textAlign', 'whiteSpace', 'wordBreak', 'overflowWrap',
                    ...
                    const markerText = (node.value || '').slice(start, Math.max(start + 1, end)) || ' ';
                    mirror.textContent = (node.value || '').slice(0, start);
                    ...
                    const markerTop = Math.max(0, marker.offsetTop - paddingTop);
                    const markerBottom = markerTop + Math.max(marker.offsetHeight, parseFloat(computed.lineHeight) || 18);
                    const viewportTop = node.scrollTop || 0;
                    const viewportBottom = viewportTop + node.clientHeight;
                    const margin = Math.max(24, (parseFloat(computed.lineHeight) || 18) * 2);
                    let targetTop = viewportTop;

                    if (markerTop - margin < viewportTop) {
                        targetTop = Math.max(0, markerTop - margin);
                    } else if (markerBottom + margin > viewportBottom - paddingBottom) {
                        targetTop = Math.max(0, markerBottom + margin - node.clientHeight + paddingBottom);
                    }

                    if (Math.abs((node.scrollTop || 0) - targetTop) > 2) node.scrollTop = targetTop;
            ...
            const selectAbcRange = useCallback((startChar, endChar) => {
                ...
                requestAnimationFrame(() => {
                    ...
                    scrollAbcSelectionIntoView(node, s, e);
                    requestAnimationFrame(() => {
                        const node2 = textAreaRef.current;
                        if (!node2) return;
                        if (node2.selectionStart !== s || node2.selectionEnd !== e) applySelection();
                        scrollAbcSelectionIntoView(node2, s, e);
                    });
                });
            }, [scrollAbcSelectionIntoView]);
            ...
            const parseMeterSignature = (meterText) => {
                const match = String(meterText || '').trim().match(/^(\d+)\s*\/\s*(\d+)$/);
                if (!match) return { numerator: 4, denominator: 4, beatUnit: 0.25, compound: false };
                const numerator = Math.max(1, parseInt(match[1], 10) || 4);
                const denominator = Math.max(1, parseInt(match[2], 10) || 4);
                const beatUnit = 1 / denominator;
                const compound = denominator === 8 && numerator >= 6 && numerator % 3 === 0;
                return { numerator, denominator, beatUnit, compound };
            };

            const clampValue = (value, min, max) => Math.max(min, Math.min(max, value));

            const getPlaybackInstrumentProfile = (instrumentName) => {
            ...
            const applyPlaybackInstrumentModel = (sourceSequence, instrumentName) => {
                if (!sourceSequence || !Array.isArray(sourceSequence.tracks)) return sourceSequence;
                const instrumentProfile = getPlaybackInstrumentProfile(instrumentName);
                const targetProgram = instrumentProfile.program;
                const styledTracks = sourceSequence.tracks.map((track) => {
                    if (!Array.isArray(track)) return track;
                    let sawProgram = false;
                    const updatedTrack = track.map((ev) => {
                        if (!ev) return ev;
                        if (ev.cmd === 'program') {
                            sawProgram = true;
                            return { ...ev, instrument: targetProgram, program: targetProgram };
                        }
                        if (ev.cmd === 'note') {
                            return { ...ev, instrument: targetProgram };
                        }
                        return { ...ev };
                    });
                    if (!sawProgram) {
                        return [{ cmd: 'program', instrument: targetProgram, program: targetProgram }, ...updatedTrack];
                    }
                    return updatedTrack;
                });
                return {
            ...
            const getPitchCenter = (ev) => {
                if (!ev || !Array.isArray(ev.pitches)) return null;
                const values = ev.pitches
                    .map((pitch) => {
                        if (typeof pitch === 'number') return pitch;
                        if (pitch && typeof pitch.pitch === 'number') return pitch.pitch;
                        if (pitch && typeof pitch.midipitch === 'number') return pitch.midipitch;
                        return null;
                    })
            ...
            const deterministicHumanize = (seed, rangeMs) => {
                const raw = Math.sin(seed * 12.9898 + 78.233) * 43758.5453;
                const frac = raw - Math.floor(raw);
                return (frac - 0.5) * 2 * rangeMs;
            };

            const buildMusicalPhrasePlan = (track, noteIndices, meterSig) => {
                const safeBeat = meterSig?.beatUnit > 0 ? meterSig.beatUnit : 0.25;
                const phraseMap = new Map();
                if (!Array.isArray(track) || !Array.isArray(noteIndices) || !noteIndices.length) return phraseMap;

                const phrases = [];
                let currentPhrase = [noteIndices[0]];

                for (let i = 1; i < noteIndices.length; i++) {
                    const prevEvent = track[noteIndices[i - 1]];
                    const currentEvent = track[noteIndices[i]];
                    const prevStart = typeof prevEvent?.start === 'number' ? prevEvent.start : 0;
                    const prevDuration = typeof prevEvent?.duration === 'number' ? prevEvent.duration : 0;
                    const currentStart = typeof currentEvent?.start === 'number' ? currentEvent.start : prevStart + prevDuration;
                    const gapWhole = currentStart - (prevStart + prevDuration);
                    const currentMeasurePos = ((currentStart % 1) + 1) % 1;
                    const prevMeasurePos = (((prevStart + prevDuration) % 1) + 1) % 1;
                    const nearMeasureRestart = currentMeasurePos < safeBeat * 0.14 && prevMeasurePos > 1 - safeBeat * 0.28;
                    const cadenceBreak = prevDuration >= safeBeat * 1.35 && nearMeasureRestart;
                    const explicitBreath = gapWhole > safeBeat * 0.24;
                    const softMeasureReset = currentPhrase.length >= 6 && nearMeasureRestart;
                    const longPhraseReset = currentPhrase.length >= 10;
                    if (explicitBreath || cadenceBreak || softMeasureReset || longPhraseReset) {
                        phrases.push(currentPhrase);
                        currentPhrase = [noteIndices[i]];
                    } else {
                        currentPhrase.push(noteIndices[i]);
                    }
                }
                if (currentPhrase.length) phrases.push(currentPhrase);

                phrases.forEach((phrase) => {
                    const pitchCenters = phrase
                        .map((noteIndex) => getPitchCenter(track[noteIndex]))
                        .filter((value) => typeof value === 'number' && Number.isFinite(value));
                    const minPitch = pitchCenters.length ? Math.min(...pitchCenters) : null;
                    const maxPitch = pitchCenters.length ? Math.max(...pitchCenters) : null;
                    const pitchSpan = minPitch != null && maxPitch != null ? Math.max(1, maxPitch - minPitch) : 1;
                    phrase.forEach((noteIndex, phraseIndex) => {
                        const count = phrase.length;
                        const t = count <= 1 ? 0 : (phraseIndex / (count - 1));
                        const arch = Math.sin(Math.PI * t);
                        const event = track[noteIndex];
                        const pitchCenter = getPitchCenter(event);
                        const pitchWeight = pitchCenter == null || minPitch == null || maxPitch == null
                            ? 0
                            : ((pitchCenter - minPitch) / pitchSpan) - 0.5;
                        phraseMap.set(noteIndex, {
                            phraseIndex,
                            phraseCount: count,
                            phraseT: t,
                            arch,
                            pitchWeight,
                            isPhraseStart: phraseIndex === 0,
                            isPhraseEnd: phraseIndex === count - 1,
                            isCadentialZone: t >= 0.72,
                        });
                    });
                });
            ...
            const computeMusicalTimingProfile = (startWhole, durationWhole, trackIndex, meterSig, msPerWhole, phraseMeta = null, pitchCenter = null, instrumentProfile = null) => {
                const { numerator, beatUnit, compound } = meterSig || { numerator: 4, beatUnit: 0.25, compound: false };
                const instrument = instrumentProfile || getPlaybackInstrumentProfile('piano');
                const safeBeat = beatUnit > 0 ? beatUnit : 0.25;
                const measurePos = ((typeof startWhole === 'number' ? startWhole : 0) % 1 + 1) % 1;

                let accentBoost = 0;

                if (compound) {
                    const groupLength = safeBeat * 3;
                    const groupIndex = Math.max(0, Math.floor((measurePos + 0.0001) / groupLength));
                    accentBoost += groupIndex === 0 ? 9 : 5;
                    const innerPulse = Math.round(((measurePos % groupLength) / safeBeat));
                    accentBoost += innerPulse === 0 ? 2 : -1;
                } else {
                    const beatIndex = Math.max(0, Math.floor((measurePos + 0.0001) / safeBeat));
                    if (beatIndex === 0) accentBoost += 10;
                    else if (numerator % 2 === 0 && beatIndex === Math.floor(numerator / 2)) accentBoost += 5;
                    else accentBoost += -1;
                }
                accentBoost *= instrument.accentWeight || 1;

                const trackBoost = (trackIndex === 0 ? 8 : (trackIndex === 1 ? 2 : -2)) + (trackIndex === 0 ? (instrument.melodyLift || 0) : 0);
                const sustainBoost = durationWhole >= safeBeat ? 3 : (durationWhole >= safeBeat * 0.5 ? 1 : 0);
                const measureTailRelax = (measurePos > (1 - safeBeat * 1.05) ? -4 : 0) * (instrument.tailWeight || 1);

                const beatPhase = safeBeat > 0 ? (measurePos / safeBeat) : 0;
                const beatFraction = beatPhase - Math.floor(beatPhase);
                const nearBeatLine = Math.abs(beatFraction) < 0.08 || Math.abs(beatFraction - 1) < 0.08;

                const phraseT = typeof phraseMeta?.phraseT === 'number' ? phraseMeta.phraseT : 0.5;
                const arch = typeof phraseMeta?.arch === 'number' ? phraseMeta.arch : 0;
                const pitchWeight = typeof phraseMeta?.pitchWeight === 'number' ? phraseMeta.pitchWeight : 0;

                const phraseVolumeBoost = (phraseMeta?.phraseCount || 0) >= 3
                    ? arch * 4.2 + pitchWeight * (trackIndex === 0 ? 5.5 : 2.5)
                    : 0;
                const phraseCadenceRelax = (phraseMeta?.isCadentialZone ? -2.5 : 0) * (instrument.tailWeight || 1);

                let shiftMs = 0;
                if (compound) {
                    const innerPulsePhase = ((measurePos % (safeBeat * 3)) / safeBeat);
                    const innerDistance = Math.abs(innerPulsePhase - Math.round(innerPulsePhase));
                    if (innerDistance > 0.12) shiftMs += (trackIndex === 0 ? 4 : 8) * (instrument.offbeatWeight || 1);
                    else if (!nearBeatLine) shiftMs += (trackIndex === 0 ? 2 : 5) * (instrument.offbeatWeight || 1);
                } else {
                    if (Math.abs(beatFraction - 0.5) < 0.16) shiftMs += (trackIndex === 0 ? 5 : 11) * (instrument.offbeatWeight || 1);
                    else if (!nearBeatLine) shiftMs += (trackIndex === 0 ? 3 : 7) * (instrument.offbeatWeight || 1);
                }

                const phraseLeadMs = (phraseMeta?.isPhraseStart ? -4 : 0) * (instrument.rubatoWeight || 1);
                const phraseRubatoMs = (phraseMeta?.phraseCount || 0) >= 4
                    ? (((phraseT < 0.22 ? -5 * (1 - phraseT / 0.22) : 0) + (phraseT > 0.7 ? (phraseT - 0.7) / 0.3 * 16 : 0)) * (instrument.rubatoWeight || 1))
                    : 0;
                const humanizeSeed = (typeof startWhole === 'number' ? startWhole : 0) * 977 + trackIndex * 37 + (typeof pitchCenter === 'number' ? pitchCenter : 0) * 13;
                const humanizeMs = nearBeatLine ? deterministicHumanize(humanizeSeed, 1.8) : deterministicHumanize(humanizeSeed, trackIndex === 0 ? 4.8 : 7.5);

                shiftMs += phraseLeadMs + phraseRubatoMs + humanizeMs;
                if (measurePos > (1 - safeBeat * 1.05)) shiftMs += 9;

                const tailLengthenMs = (durationWhole >= safeBeat ? 18 : (durationWhole >= safeBeat * 0.5 ? 8 : 0)) * (instrument.tailWeight || 1);
                const phraseTailMs = (phraseMeta?.isPhraseEnd
                    ? (durationWhole >= safeBeat ? 26 : 14)
                    : (phraseMeta?.isCadentialZone ? 6 : 0)) * (instrument.tailWeight || 1);
                const legatoMs = (durationWhole >= safeBeat * 0.5 ? 16 : 7) * (instrument.legatoWeight || 1);
                const minDurationWhole = msPerWhole > 0 ? Math.max(0.01, 20 / msPerWhole) : 0.01;

                return {
                    volumeDelta: accentBoost + trackBoost + sustainBoost + measureTailRelax + phraseVolumeBoost + phraseCadenceRelax,
                    shiftWhole: msPerWhole > 0 ? shiftMs / msPerWhole : 0,
                    tailLengthenWhole: msPerWhole > 0 ? (tailLengthenMs + phraseTailMs) / msPerWhole : 0,
                    legatoWhole: msPerWhole > 0 ? legatoMs / msPerWhole : 0,
                    minDurationWhole,
                    safeBeat,
                    phraseMeta,
                };
            ...
            const applyPlaybackStyleModel = (sourceSequence, sourceEvents, style, instrumentName, meterText, msPerWhole, totalMsHint = 0) => {
                if (!sourceSequence || !Array.isArray(sourceSequence.tracks)) {
                    return { sequence: sourceSequence, events: sourceEvents, totalMs: totalMsHint };
                }
                const instrumentProfile = getPlaybackInstrumentProfile(instrumentName);
                const instrumentReadySequence = applyPlaybackInstrumentModel(sourceSequence, instrumentName);
                const meterSig = parseMeterSignature(meterText);
                const sourceEventsList = Array.isArray(sourceEvents) ? sourceEvents.map(ev => ({ ...ev })) : [];
                const safeMsPerWhole = msPerWhole > 0 ? msPerWhole : 1;
                let computedTotalWhole = typeof instrumentReadySequence.totalDuration === 'number' ? instrumentReadySequence.totalDuration : 0;

                if (style !== 'musical') {
                    return { sequence: instrumentReadySequence, events: sourceEventsList, totalMs: totalMsHint };
                }

                const styledTracks = instrumentReadySequence.tracks.map((track, trackIndex) => {
                    if (!Array.isArray(track)) return track;
                    const noteIndices = [];
                    track.forEach((ev, idx) => {
                        if (ev && ev.cmd === 'note') noteIndices.push(idx);
                    });

                    const nextNoteIndexByIdx = new Map();
                    for (let i = 0; i < noteIndices.length; i++) {
                        nextNoteIndexByIdx.set(noteIndices[i], noteIndices[i + 1] ?? null);
                    }

                    const phrasePlan = buildMusicalPhrasePlan(track, noteIndices, meterSig);
                    let previousShiftedStart = null;

                    return track.map((ev, idx) => {
                        if (!ev) return ev;
                        if (ev.cmd !== 'note') return { ...ev };

                        const noteStart = typeof ev.start === 'number' ? ev.start : 0;
                        const durationWhole = typeof ev.duration === 'number' ? ev.duration : 0;
                        if (durationWhole <= 0) return { ...ev };

                        const phraseMeta = phrasePlan.get(idx) || null;
                        const pitchCenter = getPitchCenter(ev);
                        const profile = computeMusicalTimingProfile(noteStart, durationWhole, trackIndex, meterSig, safeMsPerWhole, phraseMeta, pitchCenter, instrumentProfile);
                        const baseVolume = typeof ev.volume === 'number' ? ev.volume : 95;
                        const nextIdx = nextNoteIndexByIdx.get(idx);
                        const nextNote = nextIdx != null ? track[nextIdx] : null;
                        const nextStart = nextNote && typeof nextNote.start === 'number' ? nextNote.start : null;
                        const gapWhole = nextStart == null ? Infinity : (nextStart - (noteStart + durationWhole));

                        let durationAdjustWhole = profile.tailLengthenWhole;
                        if (Number.isFinite(gapWhole) && gapWhole > profile.safeBeat * 0.18) {
                            const breathRatio = (phraseMeta?.isPhraseEnd ? 0.52 : 0.38) * (instrumentProfile.breathWeight || 1);
                            const breathCapMs = (phraseMeta?.isPhraseEnd ? 82 : 55) * (instrumentProfile.breathWeight || 1);
                            const breathWhole = Math.min(gapWhole * breathRatio, breathCapMs / safeMsPerWhole);
                            durationAdjustWhole -= breathWhole;
                        } else if (Number.isFinite(gapWhole) && gapWhole >= 0 && gapWhole < profile.safeBeat * 0.08) {
                            durationAdjustWhole += Math.min(profile.legatoWhole, Math.max(0, gapWhole) * 0.58 + (10 / safeMsPerWhole));
                        } else if (!Number.isFinite(gapWhole) && durationWhole >= profile.safeBeat * 0.75) {
                            durationAdjustWhole += Math.min(32 / safeMsPerWhole, durationWhole * 0.12);
                        }

                        if (phraseMeta?.isCadentialZone && !phraseMeta?.isPhraseEnd) {
                            durationAdjustWhole -= Math.min(10 / safeMsPerWhole, durationWhole * 0.06);
                        }

                        let shiftedStart = Math.max(0, noteStart + profile.shiftWhole);
                        if (previousShiftedStart != null && shiftedStart < previousShiftedStart - 0.001) {
                            shiftedStart = previousShiftedStart - 0.001;
                        }
                        previousShiftedStart = shiftedStart;

                        const shiftedDuration = Math.max(profile.minDurationWhole, durationWhole + durationAdjustWhole);
                        computedTotalWhole = Math.max(computedTotalWhole, shiftedStart + shiftedDuration);

                        return {
                            ...ev,
                            start: shiftedStart,
                            duration: shiftedDuration,
                            volume: clampValue(Math.round(baseVolume + profile.volumeDelta), 34, 124),
                        };
                    });
                });

                let lastMs = 0;
                const eventCount = sourceEventsList.filter((ev) => ev && ev.type === 'event').length;
                let eventOrdinal = 0;
                const styledEvents = sourceEventsList.map((ev, idx) => {
                    if (!ev || ev.type !== 'event') return ev;
                    const startWhole = safeMsPerWhole > 0 ? ev.milliseconds / safeMsPerWhole : 0;
                    const phraseLikeMeta = eventCount > 2 ? {
                        phraseT: eventCount <= 1 ? 0 : eventOrdinal / Math.max(1, eventCount - 1),
                        phraseCount: eventCount,
                        isPhraseStart: eventOrdinal === 0,
                        isPhraseEnd: eventOrdinal === eventCount - 1,
                        isCadentialZone: eventOrdinal >= Math.max(0, eventCount - 2),
                        arch: Math.sin(Math.PI * (eventCount <= 1 ? 0 : eventOrdinal / Math.max(1, eventCount - 1))),
                        pitchWeight: 0,
                    } : null;
                    const profile = computeMusicalTimingProfile(startWhole, meterSig.beatUnit, 0, meterSig, safeMsPerWhole, phraseLikeMeta, null, instrumentProfile);
                    let styledMs = ev.milliseconds + profile.shiftWhole * safeMsPerWhole;

                    const nextEvent = sourceEventsList[idx + 1];
                    if (nextEvent && nextEvent.type === 'event') {
                        const gapMs = nextEvent.milliseconds - ev.milliseconds;
                        if (gapMs > meterSig.beatUnit * safeMsPerWhole * 0.7) {
                            styledMs += Math.min(14, gapMs * 0.1);
                        }
                    }

                    if (styledMs < lastMs + 1) styledMs = lastMs + 1;
                    lastMs = styledMs;
                    eventOrdinal += 1;
                    return {
                        ...ev,
                        milliseconds: styledMs,
                    };
                });

                const styledTotalMs = Math.max(
                    totalMsHint || 0,
                    computedTotalWhole * safeMsPerWhole,
                    styledEvents.length ? styledEvents[styledEvents.length - 1].milliseconds : 0
                );

                return {
                    sequence: {
                        ...instrumentReadySequence,
                        tracks: styledTracks,
                        totalDuration: computedTotalWhole,
                    },
                    events: styledEvents,
                    totalMs: styledTotalMs,
                };
            };
            ...
            const getFullPlaybackPayload = () => {
                ...
                const styledModel = applyPlaybackStyleModel(sequence, playbackEvents, playbackStyle, playbackInstrument, meter, msPerWhole, totalMs);

                return {
                    key: ['full', playbackRoadmapRef.current?.key || 'base', playbackStyle, playbackInstrument, Math.round(totalWhole * 1000)].join('|'),
                    sequence: styledModel.sequence,
                    events: styledModel.events,
                    totalMs: styledModel.totalMs,
                    msPerMeasure,
                };
            };
            ...
            const getActiveRangePlayback = () => {
                ...
                const styledModel = applyPlaybackStyleModel({
                    ...sequence,
                    tracks: slicedTracks,
                    totalDuration: totalWhole,
                }, playbackEvents, playbackStyle, playbackInstrument, meter, getMillisecondsPerWholeNote(), totalMs);
            ...
            const getCurrentAbcDefaultLength = (text) => {
                const m = String(text || '').match(/^\s*L:\s*(\d+)\s*\/\s*(\d+)\s*$/m);
                const num = Number(m?.[1] || 1);
                const den = Number(m?.[2] || 8);
                return { num: num > 0 ? num : 1, den: den > 0 ? den : 8 };
            };
            ...
            const shorthandSlashForDenominator = (n) => {
                if (n === 2) return '/';
                if (n === 4) return '//';
                if (n === 8) return '///';
                if (n === 16) return '////';
            ...
            const convertHumanDurationToAbc = (durationText, fullText) => {
                if (!/^\d+$/.test(String(durationText || ''))) return String(durationText || '');
                const durationDen = Number(durationText);
                if (!Number.isFinite(durationDen) || durationDen <= 0) return String(durationText || '');

                const base = getCurrentAbcDefaultLength(fullText);
                let num = base.den;
                let den = base.num * durationDen;
                const g = gcdInt(num, den);
                num /= g;
                den /= g;

                if (den === 1) return num === 1 ? '' : String(num);
                if (num === 1) return shorthandSlashForDenominator(den);
                return `${num}/${den}`;
            };

            const convertSolfegeShortcutToken = (token, fullText) => {
                const m = String(token || '').match(/^(do|re|mi|fa|sol|la|si)(\d+(?:\/.*)?|\/.*)?$/i);
                if (!m) return null;

                const noteMap = { do: 'C', re: 'D', mi: 'E', fa: 'F', sol: 'G', la: 'A', si: 'B' };
                const syllable = String(m[1] || '').toLowerCase();
                const rawDuration = m[2] || '';
                const abcNote = noteMap[syllable];
                if (!abcNote) return null;

                let abcDuration = rawDuration;
                if (/^\d+$/.test(rawDuration)) {
                    abcDuration = convertHumanDurationToAbc(rawDuration, fullText);
                }

                return abcNote + abcDuration;
            };

            const tryExpandSolfegeShortcutOnSpace = (e) => {
                ...
                const fullText = abcTextRef.current || '';
                const before = fullText.slice(0, start);
                const match = before.match(/(?:^|[\s|\[\]{}()<>])((?:do|re|mi|fa|sol|la|si)(?:\d+(?:\/.*)?|\/.*)?)$/i);
                const rawToken = match?.[1] || '';
                if (!rawToken) return false;

                const converted = convertSolfegeShortcutToken(rawToken, fullText);
                if (!converted || converted === rawToken) return false;

                e.preventDefault();
                const replaceStart = start - rawToken.length;
                const newText = fullText.slice(0, replaceStart) + converted + ' ' + fullText.slice(end);
                applyTextChange(newText, replaceStart + converted.length + 1, { focus: true, skipHistory: false });
                return true;
            };
            ...
            const handleTextAreaKeyDown = (e) => {
                tryExpandSolfegeShortcutOnSpace(e);
            };
            ...
            const insertSmartText = (textToInsert, targetVoiceId = activeVoice) => {
                ...
                const applyBeatGroupedBeaming = (fullText, voiceId) => {
                    ...
                    const beamMeasure = (mea) => {
                        ...
                        const toks = matches.map((item) => item.tok);
                        const remainder = raw.replace(tokenRe, '').replace(/\s+/g, '');
                        tokenRe.lastIndex = 0;
                        if (remainder !== '') return raw;
                        if (!toks.length) return raw;

                        const manualJoinBefore = new Array(toks.length).fill(false);
                        for (let i = 1; i < matches.length; i++) {
                            const gap = raw.slice(matches[i - 1].end, matches[i].start);
                            if (/^\s*$/.test(gap)) {
                                manualJoinBefore[i] = gap.length === 0;
                            }
                        }
                        ...
                        for (let i = 0; i < toks.length; i++) {
                            ...
                            let boundaryBeforeCurrent = boundaryBeforeNext;
                            boundaryBeforeNext = false;

                            if (d > 0 && beatPos > eps && beatPos + d > beatLen + eps) {
                                boundaryBeforeCurrent = true;
                                beatPos = 0;
                            }
                            ...
                            if (prevTok == null) {
                                ...
                                const preserveManualJoin = manualJoinBefore[i] && isBeamable(prevTok) && isBeamable(tok);
                                if (preserveManualJoin) out += tok;
                                else if (boundaryBeforeCurrent) out += ' ' + tok;
                                else if (isBeamable(prevTok) && isBeamable(tok)) out += tok;
                                else out += ' ' + tok;
                            }
            ...
            // --- GROUPING / OVERLAY ---
            // Cách dùng: bôi đen các nốt trong khung ABC rồi bấm nút tương ứng.
            const transformSelectedNotes = (mode) => {
                const ta = textAreaRef.current;
                if (!ta) return;
                ...
                if (start === end) {
                    alert(mode === 'group'
                        ? 'Hãy bôi đen ít nhất 2 nốt trong khung ABC rồi bấm "Grouping".\nVí dụ: C E G  hoặc  C2 E2 G2'
                        : 'Hãy bôi đen ít nhất 2 nốt trong khung ABC rồi bấm "Overlay".\nVí dụ: C E G  hoặc  C2 E2 G2');
                    return;
                }
                ...
                if (tokens.length < 2) {
                    alert(mode === 'group'
                        ? 'Hãy chọn ít nhất 2 nốt để nhóm thành cột nốt.'
                        : 'Hãy chọn ít nhất 2 nốt để chèn overlay.');
                    return;
                }

                if (tokens.some(t => t.includes('|'))) {
                    alert('Vùng chọn không nên chứa dấu |. Hãy chỉ chọn các nốt cần xử lý.');
                    return;
                }

                const parseSimpleNote = (tok) => {
                    const m = tok.match(/^([\^_=]*[A-Ga-g][,']*)(\d+\/\d+|\d+|\/\d+|\/)?$/);
                    if (!m) return null;
                    return { head: m[1], len: m[2] || '' };
                };

                const parseOverlayToken = (tok) => {
                    if (/^\[[^\]]+\](\d+\/\d+|\d+|\/\d+|\/)?$/.test(tok)) return { raw: tok };
                    if (/^z(\d+\/\d+|\d+|\/\d+|\/)?$/i.test(tok)) return { raw: tok };
                    const simple = parseSimpleNote(tok);
                    if (simple) return { raw: tok, ...simple };
                    return null;
                };
                ...
                if (mode === 'group') {
                    if (tokens.some(t => t.includes('[') || t.includes(']'))) {
                        alert('Vùng chọn chỉ nên gồm các nốt rời (không gồm [ ]). Ví dụ: C E G hoặc C2 E2 G2');
                        return;
                    }

                    const parsed = tokens.map(parseSimpleNote);
                    if (parsed.some(p => !p)) {
                    ...
                    insert = `[${parsed.map(p => p.head).join('')}]${len}`;
                } else {
                    const parsed = tokens.map(parseOverlayToken);
                    if (parsed.some(p => !p)) {
                        alert('Vùng chọn phải chỉ gồm nốt / chord / rest hợp lệ để overlay. Ví dụ: C E G, [CEG] z2 hoặc C2 E2');
                        return;
                    }

                    insert = parsed.map(p => p.raw).join(' & ');
                }
            ...
            const groupSelectedToChord = () => transformSelectedNotes('group');
            const overlaySelectedNotes = () => transformSelectedNotes('overlay');
...
<\/script>
            ...
            return (
                <div className="qs-app flex flex-col h-screen">
                    ...
                    )}<header ref={headerShellRef} className="px-2 sm:px-3 py-2 bg-white border-b border-slate-200 shadow-sm shrink-0 relative z-[9999] overflow-visible">
    <div className="w-full overflow-visible" style={{ height: `${headerScaledHeight}px` }}>
        <div className="will-change-transform" style={{ transform: `scale(${headerScale})`, transformOrigin: 'top left', width: `${100 / headerScale}%` }}>
            <div ref={headerMeasureRef} className="flex items-center justify-between gap-3 min-w-max whitespace-nowrap">
                <div className="flex items-center gap-3 shrink-0 min-w-max">
                    <div className="flex items-center space-x-3 shrink-0">
                        <div className={`${headerMode === 'micro' ? 'w-9 h-9 rounded-lg' : 'w-10 h-10 rounded-xl'} bg-indigo-600 flex items-center justify-center shadow-indigo-200 shadow-lg shrink-0`}>
                            <Music size={headerMode === 'micro' ? 18 : 20} className="text-white" />
                        </div>
                        <h1 className={`${headerMode === 'micro' ? 'text-base' : headerMode === 'tiny' ? 'text-lg' : 'text-xl'} font-bold tracking-tight text-slate-900 leading-none`}>
                            Liem <span className="text-indigo-600">QuickScore</span>
                        </h1>
                    </div>
                    ...
                    <CloudControls
                        ...
                        compactHeader={headerMode !== 'full'}
                        tinyHeader={headerMode === 'tiny' || headerMode === 'micro'}
                        microHeader={headerMode === 'micro'}
                    />
                </div>
                <div className={`flex items-center bg-slate-100 rounded-lg ${headerMode === 'micro' ? 'p-0.5 gap-0.5' : headerMode === 'compact' || headerMode === 'tiny' ? 'p-0.5 gap-0.5' : 'p-1 gap-1'} shrink-0`}>
                    <button onClick={togglePlay} className={`${headerMode === 'micro' ? 'p-1.5' : 'p-2'} hover:bg-white hover:shadow-sm rounded-md text-indigo-600 transition-all`}>
                        {isPlaying ? <Pause size={headerMode === 'micro' ? 16 : 18} /> : <Play size={headerMode === 'micro' ? 16 : 18} className="ml-0.5" />}
                    </button>
                    <button onClick={stopAudio} className={`${headerMode === 'micro' ? 'p-1.5' : 'p-2'} hover:bg-white hover:shadow-sm rounded-md text-slate-600 transition-all`}>
                        <Square size={headerMode === 'micro' ? 16 : 18} />
                    </button>
                    <div className={`relative ${headerMode === 'micro' ? 'ml-0.5' : 'ml-1'} shrink-0`}>
                        <SparklesIcon size={headerMode === 'micro' ? 12 : 14} className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" />
                        <ChevronDownIcon size={headerMode === 'micro' ? 12 : 14} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-slate-400" />
                        <select
                            value={playbackStyle}
                            onChange={(e) => setPlaybackStyle(e.target.value)}
                            className={`${headerMode === 'micro' ? 'h-8 pl-7 pr-7 text-[11px] rounded-md w-[104px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'h-9 pl-8 pr-8 text-[12px] rounded-md w-[122px]' : 'h-10 pl-9 pr-9 text-sm rounded-lg w-[138px]'} appearance-none border border-slate-200 bg-white text-slate-700 font-medium shadow-sm outline-none transition-all hover:border-slate-300 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100`}
                    ...
                    <div className="relative shrink-0">
                        <Music size={headerMode === 'micro' ? 12 : 14} className="pointer-events-none absolute left-2 top-1/2 -translate-y-1/2 text-slate-400" />
                        <ChevronDownIcon size={headerMode === 'micro' ? 12 : 14} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-slate-400" />
                        <select
                            value={playbackInstrument}
                            onChange={(e) => setPlaybackInstrument(e.target.value)}
                            className={`${headerMode === 'micro' ? 'h-8 pl-7 pr-7 text-[11px] rounded-md w-[106px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'h-9 pl-8 pr-8 text-[12px] rounded-md w-[126px]' : 'h-10 pl-9 pr-9 text-sm rounded-lg w-[150px]'} appearance-none border border-slate-200 bg-white text-slate-700 font-medium shadow-sm outline-none transition-all hover:border-slate-300 focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100`}
                            title="Playback instrument"
                        >
                    ...
                    <div className={`w-px ${headerMode === 'micro' ? 'h-5 mx-0.5' : 'h-6 mx-1'} bg-slate-200`}></div>
                    <button
                        onClick={() => setLoopEnabled(v => !v)}
                        className={`group flex items-center justify-center ${headerMode === 'micro' ? 'w-8 h-8' : headerMode === 'compact' || headerMode === 'tiny' ? 'w-9 h-9' : 'w-10 h-10'} rounded-md border transition-all ${loopEnabled ? 'bg-indigo-600 text-white border-indigo-500 shadow-[0_0_0_1px_rgba(99,102,241,0.08),0_0_18px_rgba(99,102,241,0.28)]' : 'qs-toolbar-ghost border-transparent hover:shadow-sm'}`}
                        title={loopEnabled ? 'Loop đang bật' : 'Loop đang tắt'}
                    >
                        <RotateCcwIcon size={headerMode === 'micro' ? 13 : 15} className={`transition-transform ${loopEnabled ? '-rotate-45' : 'opacity-80 group-hover:-rotate-45'}`} />
                    </button>
                    <button
                        onClick={() => { if (hasRangeAnchor) clearSelectedRange(); }}
                        className={`${headerMode === 'micro' ? 'px-2 py-1.5 text-[11px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'px-2.5 py-1.5 text-[12px]' : 'px-3 py-2 text-xs'} font-semibold rounded-md transition-all ${hasActiveRange ? 'bg-white text-emerald-700 shadow-sm' : hasRangeAnchor ? 'bg-white text-amber-700 shadow-sm' : 'text-slate-400 opacity-60 hover:bg-white/60'}`}
                ...
                <div className="flex items-center justify-end gap-2 shrink-0">
                    <button onClick={handleUndo} disabled={undoStack.length === 0} title="Undo (tối đa 5 bước)" className={`${headerMode === 'micro' ? 'p-1.5' : 'p-2'} rounded-lg border border-slate-200 bg-white text-slate-700 transition-all ${undoStack.length === 0 ? 'opacity-40 cursor-not-allowed' : 'hover:bg-slate-100 hover:shadow-sm'}`}>
                        <UndoIcon size={headerMode === 'micro' ? 16 : 18} />
                    </button>
                    <button onClick={handleRedo} disabled={redoStack.length === 0} title="Redo (tối đa 5 bước)" className={`${headerMode === 'micro' ? 'p-1.5' : 'p-2'} rounded-lg border border-slate-200 bg-white text-slate-700 transition-all ${redoStack.length === 0 ? 'opacity-40 cursor-not-allowed' : 'hover:bg-slate-100 hover:shadow-sm'}`}>
                        <RedoIcon size={headerMode === 'micro' ? 16 : 18} />
                    </button>
                    <div className="relative z-[9999] shrink-0" data-export-root>
                        <button
                            onClick={() => setShowExportMenu(v => !v)}
                            className={`${headerMode === 'micro' ? 'px-2.5 py-1.5 text-[12px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'px-3 py-1.5 text-[13px]' : 'px-4 py-2 text-sm'} bg-white hover:bg-slate-50 text-slate-800 font-medium rounded-lg transition-colors flex items-center border border-slate-200 shadow-sm`}
                            title="Export"
                        >
                            <DownloadIcon size={headerMode === 'micro' ? 14 : 16} className={headerMode === 'micro' ? 'mr-1.5 text-slate-700' : 'mr-2 text-slate-700'} />
                            {headerMode === 'micro' ? 'Exp' : t('export')}
                        </button>
                    ...
                    <div className={`flex items-center gap-1 ${headerMode === 'micro' ? '' : 'pl-1'} shrink-0`}>
                        {headerMode !== 'micro' && <GlobeIcon size={16} className="text-slate-500" />}
                        <select
                            value={lang}
                            onChange={(e) => setLang(e.target.value)}
                            className={`${headerMode === 'micro' ? 'w-[72px] px-1.5 py-1.5 text-[12px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'w-[84px] px-2 py-1.5 text-[13px]' : 'px-3 py-2 text-sm'} bg-white border border-slate-200 rounded-lg text-slate-700 outline-none hover:bg-slate-50`}
                            title="Language"
                        >
                    ...
                    <div className={`flex items-center gap-1 ${headerMode === 'micro' ? '' : 'pl-1'} shrink-0`}>
                        {headerMode !== 'micro' && <PaletteIcon size={16} className="text-slate-500" />}
                        <select
                            value={theme}
                            onChange={(e) => setTheme(e.target.value)}
                            className={`${headerMode === 'micro' ? 'w-[74px] px-1.5 py-1.5 text-[12px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'w-[88px] px-2 py-1.5 text-[13px]' : 'px-3 py-2 text-sm'} bg-white border border-slate-200 rounded-lg text-slate-700 outline-none hover:bg-slate-50`}
                            title="Theme"
                        >
                    ...
                    <button
                        ...
                        disabled={importBusy}
                        className={`${headerMode === 'micro' ? 'px-2 py-1.5 text-[12px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'px-2 py-1.5 text-[13px]' : 'px-2 py-2 text-sm'} underline underline-offset-4 rounded-lg transition-colors ` + (importBusy ? "text-slate-400 cursor-not-allowed" : "text-slate-700 hover:text-slate-900 hover:bg-slate-100")}
                        title="Import PDF / ảnh / MXL / MIDI"
                    >
                        {importBusy ? 'Importing…' : (headerMode === 'micro' ? 'Imp' : 'Import')}
                    </button>
                    ...
                    <button
                        ...
                        className={`${headerMode === 'micro' ? 'px-2 py-1.5 text-[12px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'px-2 py-1.5 text-[13px]' : 'px-2 py-2 text-sm'} text-slate-700 underline underline-offset-4 hover:text-slate-900 hover:bg-slate-100 rounded-lg transition-colors`}
                        title="Xem tất cả hotkeys"
                    >
                        {headerMode === 'micro' ? 'Keys' : 'Hotkeys'}
                    </button>

                    <div className="relative z-[9999] shrink-0" data-optimize-root>
                        <button
                            onClick={() => setShowOptimizeMenu(v => !v)}
                            className={`${headerMode === 'micro' ? 'px-3 py-1.5 text-[12px]' : headerMode === 'compact' || headerMode === 'tiny' ? 'px-3.5 py-1.5 text-[13px]' : 'px-5 py-2 text-sm'} bg-slate-900 hover:bg-slate-800 text-white font-medium rounded-lg transition-colors flex items-center shadow-md shadow-slate-200`}
                            title="Optimize"
                        >
                            <SparklesIcon size={headerMode === 'micro' ? 14 : 16} className={headerMode === 'micro' ? 'mr-1.5' : 'mr-2'} />
                            {headerMode === 'micro' ? 'Opt' : 'Optimize'}
                        </button>
                    ...
                    <div className="flex flex-1 overflow-hidden p-4 gap-4 w-full">
                        <div className={previewExpanded ? "hidden" : "w-[560px] 2xl:w-[620px] flex flex-col gap-3 min-w-[360px]"}>
                            <div className="flex h-2/5 gap-3">
                                <div className="w-1/2 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden">
                                    ...
                                    <div className="p-3 space-y-3 overflow-y-auto">
                                        <div className="space-y-3">
                                            <input type="text" value={title} placeholder="Tên bài hát..." onChange={e => setTitle(e.target.value)} onBlur={(e) => updateHeaderField('T', e.target.value)} className="qs-info-field w-full text-sm border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all" />
                                            <div className="flex space-x-2">
                                                <input type="text" value={composer} placeholder="Composer" onChange={e => setComposer(e.target.value)} onBlur={(e) => updateHeaderField('C', e.target.value)} className="qs-info-field w-1/2 text-sm border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all" />
                                                <select value={meter} onChange={e => { setMeter(e.target.value); updateHeaderField('M', e.target.value); }} className="qs-info-field w-1/4 text-sm border border-slate-200 p-2 rounded-lg outline-none cursor-pointer focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400">
                                                    <option value="4/4">4/4</option>
                                                    <option value="3/4">3/4</option>
                                                    <option value="2/4">2/4</option>
                                                    <option value="6/8">6/8</option>
                                                </select>
                                                <input type="number" value={tempo} placeholder="BPM" title="Tempo (BPM)" onChange={e => setTempo(e.target.value)} onBlur={(e) => updateHeaderField('Q', `1/4=${e.target.value}`)} className="qs-info-field w-1/4 text-sm border border-slate-200 p-2 rounded-lg focus:ring-2 focus:ring-indigo-100 focus:border-indigo-400 outline-none transition-all" />
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-2 border-t border-slate-100">
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <span className="qs-info-chip text-[11px] px-2 py-1 rounded-full border border-slate-200 bg-white text-slate-600">
                                                        Dur: {durationToPretty(duration)}{isDotted ? ' • dôi' : ''}
                                                    </span>
                                                    <button
                                                        type="button"
                                                        onClick={() => setShowHotkeysHelp(v => !v)}
                                                        className="qs-info-chip w-7 h-7 flex items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 hover:bg-slate-50"
                                                ...
                                                <div className="relative group flex-1">
                                                    <div className="flex gap-2">
                                                        <button
                                                            ...
                                                            className="flex-1 px-3 py-2 text-xs font-medium rounded-lg border bg-white border-slate-200 text-slate-600 hover:bg-slate-50 transition-all"
                                                            title="Nhóm các nốt đã bôi đen thành 1 cột nốt (chord)"
                                                        >
                                                            Grouping
                                                        </button>
                                                        <button
                                                            onClick={overlaySelectedNotes}
                                                            className="flex-1 px-3 py-2 text-xs font-medium rounded-lg border bg-white border-slate-200 text-slate-600 hover:bg-slate-50 transition-all"
                                                            title="Chèn ký hiệu overlay (&) giữa các nốt đã bôi đen"
                                                        >
                                                            Overlay
                                                        </button>
                                                    </div>
                                                    ...
                                                    <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-full mt-2 w-80 bg-slate-900 text-white text-[11px] leading-snug rounded-lg px-3 py-2 shadow-xl z-50 opacity-0 group-hover:opacity-100 transition-opacity">
                                                        ...
                                                        <div>
                                                            ...
                                                            <br />
                                                            <span className="font-semibold">Grouping</span> → gộp thành <span className="font-mono">[CEG]</span>.
                                                            <br />
                                                            <span className="font-semibold">Overlay</span> → chèn <span className="font-mono">&amp;</span> ở giữa thành <span className="font-mono">C &amp; E &amp; G</span>.
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div className="w-1/2 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden relative">
                                    ...
                                    <textarea ref={textAreaRef} value={abcText} onChange={handleTextAreaChange} onKeyDown={handleTextAreaKeyDown} onClick={handleTextAreaClickOrKey} onKeyUp={handleTextAreaClickOrKey} className="flex-1 w-full p-3 resize-none outline-none font-mono text-sm bg-transparent text-slate-700 leading-relaxed" spellCheck="false" />
                                </div>
                            </div>
                            ...
                            <div className="flex-1 bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col overflow-hidden relative">
                                ...
                                <div className="flex-1 p-4 overflow-y-auto select-none bg-white">
                                    ...
                                    <div className="space-y-2 pt-4">
                                        {OCTAVES.map(oct => {
                                            const activeNotes = Array.isArray(oct.notes) && oct.notes.length ? oct.notes : NOTES;
                                            return (
                            ...
                            <div className="bg-slate-50 border-b border-slate-200 px-4 py-2 flex justify-between items-center z-10">
                                <div className="flex items-center gap-2 text-slate-700">
                                    <button
                                        ...
                                        className="w-7 h-7 flex items-center justify-center rounded-md border border-slate-200 bg-white text-slate-700 hover:bg-slate-100 transition-all"
                                        title={previewExpanded ? "Thu về chế độ thường" : "Mở rộng full khung sheet nhạc"}
                                        aria-label={previewExpanded ? "Thu về chế độ thường" : "Mở rộng full khung sheet nhạc"}
                                    >
                                        <span className="text-sm font-bold leading-none">{previewExpanded ? '>' : '<'}</span>
                                    </button>
